...
@Component
public class PromptValidator implements ValidationRule {

    @Override
    public List<ValidationIssue> validate(BootstrapWorld world) {
        List<ValidationIssue> issues = new ArrayList<>();
        for (BootstrapFile file : world.prompts()) {
            if (file.metadata().isEmpty()) {
                continue;
            }
            validatePromptMetadata(file.metadata().get(), issues);
        }
        return issues;
    }

    private void validatePromptMetadata(DocumentMetadata metadata, List<ValidationIssue> issues) {
        if (metadata.id() == null || metadata.id().isBlank()) {
            issues.add(error("Prompt missing required field: id", metadata.filePath()));
        } else if (!metadata.id().startsWith("prompt_")) {
            issues.add(error("Prompt id must start with 'prompt_'", metadata.filePath()));
        }
        if (metadata.title() == null || metadata.title().isBlank()) {
            issues.add(error("Prompt missing required field: title", metadata.filePath()));
        }
        if (metadata.version() == null || metadata.version().isBlank()) {
            issues.add(error("Prompt missing required field: version", metadata.filePath()));
        }
        if (metadata.status() == null || metadata.status().isBlank()) {
            issues.add(error("Prompt missing required field: status", metadata.filePath()));
        }
    }
